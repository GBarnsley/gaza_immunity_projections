...
df %>%
    ggplot(aes(x = Date, y = `Basic Reproduction Number`, colour = Pathogen)) +
    geom_line() +
    ggpubr::theme_pubclean() +
    scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
    coord_cartesian(ylim = c(0, max(df$`Basic Reproduction Number`))) +
    geom_vline(xintercept = date_projection_start, linetype = "dashed") +
    labs(x = "Date", y = "Basic Reproduction Number")
```
